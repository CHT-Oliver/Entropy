...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def get_rope_index(
        ...
        video_time_grid_ts: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        Args:
            ...
            video_time_grid_ts (`torch.Tensor` of shape `(num_videos, max_grid_t)`, *optional*):
                Explicit timestamp in seconds for each video temporal grid. When provided, it overrides
                `second_per_grid_ts` for videos and allows non-uniform temporal sampling.
            attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask to avoid performing attention on padding token indices. Mask values selected in `[0, 1]`:
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    if ed_image < ed_video:
                        ...
                        video_time_grid_t = (
                            video_time_grid_ts[video_index] if video_time_grid_ts is not None else None
                        )
                        video_index += 1
                        remain_videos -= 1
                        ed = ed_video
                    ...
                    if video_time_grid_t is not None:
                        video_time_grid_t = torch.as_tensor(
                            video_time_grid_t, dtype=torch.float32, device=range_tensor.device
                        ).flatten()
                        if video_time_grid_t.numel() < llm_grid_t:
                            raise ValueError(
                                f"video_time_grid_ts for video {video_index - 1} has {video_time_grid_t.numel()} "
                                f"timestamps, but this video requires {llm_grid_t} temporal grids."
                            )
                        time_tensor = video_time_grid_t[:llm_grid_t].view(-1, 1).expand(-1, llm_grid_h * llm_grid_w)
                        time_tensor_long = torch.round(
                            time_tensor * self.config.vision_config.tokens_per_second
                        ).long()
                    else:
        ...
        model_inputs.update(
            {
                ...
                "video_time_grid_ts": video_time_grid_ts,
            }
        )
        return model_inputs
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            for key in dict_to_expand:
                ...
                elif key == "video_time_grid_ts":
                    lengths = list(video_nums)
                    if not isinstance(dict_to_expand[key], torch.Tensor):
                        dict_to_expand[key] = torch.tensor(dict_to_expand[key])
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
